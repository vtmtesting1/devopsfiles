                ...
                stage ('API Functional Test') {
                    if(config.apiFunctionalTest == true) {
                        dir(config.MicroserviceName) {
                            try {
                                retry(3) {
                                    sh 'gradle apifunctionaltest -DtestEnv=dev -Dintegrationurl=https://m-int1.walgreens.com --debug'                                 
                                }
                            } catch (exc) {
                                currentBuild.result = "FAILURE"
                                throw exc
                    ...
                    if(config.mwsApiFunctionalTest == true) {
                        dir(config.MicroserviceName) {
                            try {
                                retry(3) {
                                    sh 'gradle mwsApiFunctionalTest -DtestEnv=dev -Dintegrationurl=https://m-int1.walgreens.com'
                                }
                            } catch (exc) {
                                currentBuild.result = "FAILURE"
                                throw exc
