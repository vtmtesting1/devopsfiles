...
def call(body) {
	...
	podTemplate(
		...
		containers: [
			containerTemplate(
				name: label,
				image: 'nonprodregistry.azurecr.io/baseimg/wag-dotcom-functional-test-zulu10-nodejs10-bionic:v1',
				command: 'cat',
				ttyEnabled: true,
			...
			container(label) {
				stage('Clone') {
					dir(config.MicroserviceName) {
						try {
							...
							curl "http://172.17.65.9:8080/view/Microservices/job/${config.MicroserviceName}/job/${config.MicroserviceName}-build-qa3/lastSuccessfulBuild/api/xml?xpath=.//action/parameter\\[1\\]/value" | awk -F'>' '{print \$2}' | awk -F'<' '{print \$1}'
							""").trim()
							echo FeatureBranch
				...
				stage ('API Functional Test') {
					if(config.apiFunctionalTest == true) {
						dir(config.MicroserviceName) {
							try {
								def functionalTestHost = 'dotcom-acs-qa3.walgreens.com'
