		...
		timeout(120) {
			node(label) {
				container(label) {
					...
					def imageTag = "gcpdev01akshelm.azurecr.io/digital/${application}/${module}:${env.BUILD_NUMBER}";
					...
					stage('Clone') {
						try {
                            ...
                            if (timeStamp >= 11 && timeStamp < 17) {
                                triggerRemoteJob maxConn: 5, useCrumbCache: true, useJobInfoCache: true, abortTriggeredJob: true, auth: TokenAuth(apiToken: '776b3fa981c61046348a178df3074d55', userName: 'admin'), blockBuildUntilComplete: false, job: 'trigger-email', parameters: "MicroserviceName=${config.MicroserviceName}", remoteJenkinsUrl: 'http://dgtljenkins-acs.walgreens.com:8080'
                            }*/
							dir('scripts') {
							...
							dir(config.MicroserviceName) {
								try {
									...
									withDockerRegistry([credentialsId: 'Docker_Cred', url: 'https://nonprodregistry.azurecr.io']) {
										sh "docker pull nonprodregistry.azurecr.io/baseimg/nodejs12_microservices_bionic:v1"
									}
									withDockerRegistry([credentialsId: 'Docker_Cred', url: 'https://nonprodregistry.azurecr.io']) {
					...
					if(params.NodePublish != '' && params.NodePublish == true) {
                  ...
                  if (params.NodePublish != true) {
						stage('Push to Boots ACR') {
							dir(config.MicroserviceName) {
								try {
									sh "docker tag ${module} '${imageTag}'"
									withDockerRegistry([credentialsId: 'Boots_Docker_Cred', url: 'https://gcpdev01akshelm.azurecr.io']) {
